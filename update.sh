...
# Use local template for testing, or pass path as argument: ./update.sh path/to/template
...
CURRENT_VERSION=$(grep '^version=' "$TEMPLATE_PATH" | cut -d= -f2)
...
# Make sure to decompress the JS bundle
...
echo "Local template successfully updated to $NEW_VERSION."
